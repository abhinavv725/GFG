...
class Tree {
    ...
    private void serialHelper(Node root, ArrayList<Integer> ans){
        if(root==null){
            ans.add(null);
            return;
        }
    ...
    private Node deserialHelper(ArrayList<Integer> data,Index i){
        if(i.idx>=data.size() || data.get(i.idx)==null){
            i.idx++;
            return null;
        }
        Node root = new Node(data.get(i.idx));
        i.idx++;
