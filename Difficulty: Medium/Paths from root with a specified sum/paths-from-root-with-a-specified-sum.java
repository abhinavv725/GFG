...
class Solution
{
    private static void helper(Node root, int sum,ArrayList<ArrayList<Integer>> ans,ArrayList<Integer> temp,int[] s){
        if(root!=null){
            s[0]+=root.data;
            temp.add(root.data);
            if(s[0]==sum){
            ...
            helper(root.left,sum, ans, temp, s);
            helper(root.right,sum, ans, temp, s);
    ...
    public static ArrayList<ArrayList<Integer>> printPaths(Node root, int sum)
    {
        ...
        helper(root, sum, ans, temp,new int[1]);
